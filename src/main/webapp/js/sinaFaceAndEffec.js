...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			$('#mycontainer').append('<div id="emotions"></div>');
			//$('#emotions').css({top: eTop, left: eLeft});
		...
		function showEmotions(){
			...
			for(var i = 0;  i < emotions[category].length; ++i){
				$('#emotions .container1').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></a>'));
			}
