...
Promise.all(assets).then( function(images) {
	...
	$(".thumbUp").click( function(event)
	{
		...
		// console.log("点赞clicked  aidData: "+aid);

		var alreadyDone = window.localStorage.getItem(aid);
		if ( alreadyDone == null || alreadyDone == ""  ){	//如果该文章没有点过赞

			window.localStorage.setItem(aid,"true");
			//执行  冒爱心
			var innerHeight = window.innerHeight;
			var clientH = document.documentElement.clientHeight;
			//console.log("innerH "+innerHeight+"  clientH "+clientH);
			...
			var newBottom = innerHeight-event.clientY+35;
			var newRight = document.documentElement.clientWidth-event.clientX-70;

			$("canvas").css( {"bottom":newBottom,"right":newRight} );
			...
			$(document.getElementById(aid)).tooltip({
				title: "已经点过赞了",
				placement: "right",
				trigger: "manual"
			});

			$(document.getElementById(aid)).tooltip('show');

			setTimeout(function () {
				$(document.getElementById(aid)).tooltip('hide');
			},1500);
		}
